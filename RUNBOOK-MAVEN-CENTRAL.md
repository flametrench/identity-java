...
This document describes how to publish `dev.flametrench:identity` to Maven Central. The same flow applies to `dev.flametrench:{authz,tenancy,identity}` — each is a sibling repo with the same `release` profile in its `pom.xml`.
